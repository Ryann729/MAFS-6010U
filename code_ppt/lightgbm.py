...
def simple_fe(data):
    ...
    data['rolling_std_t14'] = data.groupby(['id'])['demand'].transform(lambda x: x.shift(28).rolling(14).std())
    ...
    data['lag_price_t14'] = data.groupby(['id'])['sell_price'].transform(lambda x: x.shift(14))
    data['lag_price_t30'] = data.groupby(['id'])['sell_price'].transform(lambda x: x.shift(30))
    data['lag_price_t90'] = data.groupby(['id'])['sell_price'].transform(lambda x: x.shift(90))
    data['lag_price_t180'] = data.groupby(['id'])['sell_price'].transform(lambda x: x.shift(180))
    ...
    data['price_change_t14'] = (data['lag_price_t14'] - data['sell_price']) / (data['lag_price_t14'])
    data['price_change_t30'] = (data['lag_price_t30'] - data['sell_price']) / (data['lag_price_t30'])
    data['price_change_t90'] = (data['lag_price_t90'] - data['sell_price']) / (data['rolling_price_max_t90'])
    ...
    data['rolling_price_std_t14'] = data.groupby(['id'])['sell_price'].transform(lambda x: x.rolling(14).std())
...
columns = ['item_id', 'dept_id', 'cat_id', 'store_id', 'state_id', 'year', 'month', 'week', 'day', 'dayofweek', 'event_name_1', 'event_type_1', 'event_name_2', 'event_type_2', 
            'snap_CA', 'snap_TX', 'snap_WI', 'sell_price', 'lag_t28', 'lag_t29', 'lag_t30', 'rolling_mean_t7', 'rolling_std_t7','rolling_mean_t14', 'rolling_std_t14', 'rolling_mean_t30',
           'rolling_std_t30', 'rolling_mean_t90', 'rolling_std_t90','rolling_mean_t180', 'rolling_std_t180', 'price_change_t365', 'rolling_price_std_t7','rolling_price_std_t14' ,
           'rolling_price_std_t30','rolling_price_std_t90','rolling_price_std_t180']
...
def predict(test, submission):
    ...
    predictions.columns = ['id'] + ['F' + str(i + 1) for i in range(predictions.shape[1]-1)]
...
plt.title(' All feature importance over {} folds average'.format(folds.n_splits));
